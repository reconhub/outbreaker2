...
test_that("test: settings are processed fine", {
  ...
  expect_equal(
    create_config(init_tree = alpha)$init_tree,
...
test_that("move_alpha recycles only when length 1, else must match N", {
  ...
  ## wrong length errors
  expect_error(
    create_config(data = dat, move_alpha = c(TRUE, FALSE)),
