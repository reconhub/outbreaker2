...
custom_likelihoods(genetic = function(x) {
  0.0
})
```
...
```{r, wt_custom}
onset_data <- outbreaker_data(
  dates = fake_outbreak$onset,
  w_dens = fake_outbreak$w
)

wt_model <- custom_likelihoods(
  timing_sampling = f_null,
  reporting = f_null
)
```
...
```{r, wt_config}
wt_config <- create_config(
  init_kappa = 1, move_kappa = FALSE,
