...
.ll_genetic <- function(data, param, i = NULL) {
  ...
  out <- sum(
    n_mut * log(param$kappa[i] * param$mu) +
...
.ll_timing_infections <- function(data, param, i = NULL) {
  ...
  delay <- param$t_inf[i] - param$t_inf[param$alpha[i]]
  ...
  if (any(delay < 1 | delay > ncol(data$log_w_dens), na.rm = TRUE)) {
    return(-Inf)
  }

  ## output is a sum of log-densities
  sum(data$log_w_dens[cbind(param$kappa[i], delay)], na.rm = TRUE)
}
...
.ll_timing_sampling <- function(data, param, i = NULL) {
  ...
  ## compute delays
  delay <- data$dates[i] - param$t_inf[i]
  delay <- delay[!is.na(delay)]

  ## avoid over-shooting
  if (any(delay < 1 | delay > length(data$log_f_dens))) {
    return(-Inf)
  }

  ## output is a sum of log densities
  sum(data$log_f_dens[delay], na.rm = TRUE)
}
