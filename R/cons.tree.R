...
## #'
## #' The S3 class \code{cons.tree} is used for storing simplified transmission trees,
## #' obtained from outbreaker's ouptput (functions \code{outbreaker} and
## #' \code{outbreaker.parallel}) using \code{get.cons.tree}. Some additional features
## #' are available for \code{cons.tree} objects, including plotting (\code{plot}),
...
## #'
## #'
## #' @aliases cons.tree get.cons.tree plot.cons.tree as.igraph.cons.tree findMutations.cons.tree
## #'
## #' @export
## #'
## #' @rdname cons.tree
## #'
## #' @param x for \code{get.cons.tree}, the output of \code{outbreaker} or
## #' \code{outbreaker.parallel}. For other functions, a \code{cons.tree} object.
...
## #' @return \code{cons.tree} objects are lists with the following components:
...
## #'
## #' The plot function invisibly returns the conversion of the \code{cons.tree}
...
## #'
## #'
## cons.tree <- function(x, burnin=2e4, best=c("ancestries","tree")){
...
## ## #' @export
## ## #'
## ## as.igraph.cons.tree <- function(x, edge.col="black", col.edge.by="prob",
...
## ##     if(!inherits(x,"cons.tree")) stop("x is not a cons.tree object")
...
## ## #' @export
## ## #'
## ## findMutations.cons.tree <- function(x, dna, ...){
## ##     ## CHECKS ##
...
## ## #' @export
## ## #'
## ## plot.cons.tree <- function(x, y=NULL, edge.col="black", col.edge.by="prob",
## ##                               col.pal=NULL, annot=c("dist","n.gen","prob"), sep="/", ...){
