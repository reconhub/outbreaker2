...
print.outbreaker_chains <- function(x, n_row = 3, n_col = 8, ...) {
  ...
  if (ncol(x) > n_col) {
    ...
    alpha_txt <- paste(
      not_shown[range(grep("alpha", not_shown))],
      collapse = " - "
    )
    t_inf_txt <- paste(
      not_shown[range(grep("t_inf", not_shown))],
      collapse = " - "
    )
    kappa_txt <- paste(
      not_shown[range(grep("kappa", not_shown))],
      collapse = " - "
    )
...
#'
plot.outbreaker_chains <- function(
  x,
  y = "post",
  ...
  if (type == "alpha") {
    ...
    N <- ncol(alpha)
    ## if return_ids, map character IDs back to integers for xyTable
    if (isTRUE(attr(x, "return_ids"))) {
      ids <- attr(x, "ids")
      id_to_int <- setNames(seq_along(ids), ids)
      alpha[] <- id_to_int[alpha]
    ...
    out_dat$from <- factor(
      out_dat$from,
      levels = c(0, sort(unique(out_dat$to)))
    )
  ...
  if (type == "t_inf") {
    ...
    get_lab_color <- function(labels = NULL, color = NULL) {
      list(
        t_inf_lab_x = get_t_inf_lab(labels),
  ...
  if (type == "kappa") {
    ...
    out <- ggplot(out_dat) +
      ...
      guides(colour = "none") +
      labs(
        title = "number of generations between cases",
  ...
  if (type == "network") {
    ...
    if (isTRUE(attr(x, "return_ids"))) {
      ids <- attr(x, "ids")
      id_to_int <- setNames(seq_along(ids), ids)
      alpha[] <- lapply(alpha, function(col) id_to_int[col])
    }
    from <- unlist(alpha)
    to <- as.vector(col(alpha))
    N <- ncol(alpha)
    ...
    nodes <- data.frame(id = seq_len(ncol(alpha)), label = seq_len(ncol(alpha)))
    nodes$value <- vapply(nodes$id, find_nodes_size, numeric(1))
...
summary.outbreaker_chains <- function(
  object,
  burnin = 0,
  method = c("mpa", "decycle"),
  ...
) {
  ## check burnin ##
  x <- object
  ...
  ## summary of alpha ##
  alpha <- as.matrix(x[, grep("alpha", names(x))])
  return_ids <- isTRUE(attr(x, "return_ids"))
  ...
  if (method == 'mpa') {
    ...
    if (return_ids) {
      f1 <- function(x) {
        names(sort(table(x, exclude = NULL), decreasing = TRUE))[1]
      }
    } else {
      f1 <- function(x) {
  ...
  ## translate indexes to original case IDs (skip if already translated)
  ids <- attr(x, "ids")
  if (!is.null(ids) && !return_ids) {
    out$tree$from <- ids[out$tree$from]
    out$tree$to <- ids[out$tree$to]
  }
