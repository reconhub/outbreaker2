...
test_that("results ok: no missing cases, detect imported cases", {
  ...
  ## analysis
  set.seed(1)
  out_with_import <- outbreaker(
    data = list(
      dna = x$dna, dates = x$onset,
      w_dens = x$w
    ),
    config = list(
      n_iter = 5000, sample_every = 100,
      init_tree = "star",
...
test_that("results ok: kappa and pi", {
  ...
  config <- list(
    n_iter = 5000, sample_every = 50, init_tree = "star",
